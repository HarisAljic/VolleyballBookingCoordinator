...
export async function renderJoin() {
  ...
  layout("Join with run code", `
      ...
      <form id="form-join" class="max-w-md space-y-4">
        <input name="code" required maxlength="8" placeholder="e.g. ABC12X" class="w-full uppercase rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-100 outline-none focus:ring-2 focus:ring-emerald-500/40"${pre} />
        <button type="submit" class="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-500">Join run</button>
      </form>`,
    { variant: "form" }
  );
